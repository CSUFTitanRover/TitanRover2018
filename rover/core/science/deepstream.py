...
def get(recordName, ip="192.168.1.2"):
    ...
    payload = {"body": [
        {"topic": "record", "action": "read", "recordName": "rover/" + recordName}]}
...
def post(obj, recordName, ip="192.168.1.2"):
    '''
        This function will post the object sent to the deepstream server.
    ...
    payload = {"body": [{"topic": "record", "action": "write",
        "recordName": "rover/" + recordName, "data": obj}]}
    request = requests.post('http://' + roverIp + ':4080', json=payload)
...
def publish(eventName, data, ip="localhost"):
    '''
        This function will publish an event the deepstream server.

        Arguments:
            eventName: the unique event name that will be published to
            data: the data payload to be delivered along with the published event
    '''

    roverIp = ip
    if type(eventName) is not str:
        raise "Your first argument needs to be a string when setting the event name."

    payload = {"body":[{"topic": "event", "action":"emit", "eventName": "rover/" + eventName, "data": data}]}
    request = requests.post('http://' + roverIp + ':4080', json=payload)
    if request is not None:
        if type(request) is bytes:
            request = request.decode('utf-8')    
