...
class DegreesDecimalMinutesInput extends Component {
  static propTypes = {
  ...
  handleLatitudeChange = ({ target }) => {
    ...
    const { name } = target;
    if (name === 'decimal-minutes') {
      // parse the values
      const value = parseFloat(target.value);
      const decimalMinutes = value - Math.floor(value);
      let [minutes] = target.value.split('.');
      minutes = parseInt(minutes, 10);
      const seconds = decimalMinutes * 60;

      latitude.minutes = minutes.toString();
      latitude.seconds = seconds.toString();
    } else {
      latitude[name] = target.value;
    }
  ...
  handleLongitudeChange = ({ target }) => {
    ...
    if (name === 'decimal-minutes') {
      ...
      longitude.minutes = minutes.toString();
      longitude.seconds = seconds.toString();
    } else {
      longitude[name] = target.value;
    }
  ...
  render() {
    const labels = ['degrees', 'decimal-minutes'];
    const placeholders = ['DDD°', 'MM.MMM\''];
...
export default withStyles(styles)(DegreesDecimalMinutesInput);
